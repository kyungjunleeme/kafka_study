...
- 이러한 특징으로 카프카 브로커를 운영할 때 힙 메모리를 6GB 이상으로 설정하지 않는 것이 일반적
- 영상보고 5GB -> 6GB 로 수정
- cf) https://www.youtube.com/watch?v=kGYdLiPzTOI
...
cf) https://stackoverflow.com/questions/58224758/what-does-it-really-mean-by-kafka-partitions-are-immutable
https://stackoverflow.com/questions/37914250/hadoop-immutable-vs-data-type-mutable


# Kafka 설정: Offset Commit 및 ZooKeeper 관련 설정

## 1. Offset Commit 주기 관련 설정 (Consumer Config)
Consumer가 offset을 ZooKeeper나 Kafka에 주기적으로 커밋하는 시간을 조정합니다.

- **`auto.commit.interval.ms`**
  - Offset을 커밋하는 주기를 밀리초 단위로 설정.
  - 기본값: `5000` (5초)
  - 1분으로 설정:
    ```properties
...
## 2. ZooKeeper 기반 Offset 관리 (Deprecated)
Kafka 0.9 이전에는 ZooKeeper에 Offset을 저장했으나, 최신 Kafka에서는 **`__consumer_offsets`** 토픽 사용 권장.

- **`offset.storage`** (Deprecated)
  - Offset 저장 위치 지정 (`zookeeper` 또는 `kafka`).
  - ZooKeeper에 저장하려면:
    ```properties
    offset.storage=zookeeper
    ```

- **`dual.commit.enabled`** (Deprecated)
  - Offset을 Kafka와 ZooKeeper 양쪽에 저장할지 여부.
  - 기본값: `true`
  - 최신 Kafka에서는 Kafka만 사용:
    ```properties
...
## 3. Consumer Group Rebalance 관련 설정
Consumer가 실패할 경우, 메시지를 다시 읽는 데 걸리는 시간 조정.

- **`session.timeout.ms`**
  - Consumer group에서 heartbeat가 끊어질 때까지 기다리는 시간.
  - 기본값: `10000` (10초)
  - 1분으로 설정:
    ```properties
...
- **`heartbeat.interval.ms`**
  - Consumer가 주기적으로 heartbeat를 보낼 간격.
  - 기본값: `3000` (3초)
...
## 4. 최신 Kafka와 ZooKeeper 관련성
Kafka 0.9 이후:
- **ZooKeeper 대신 Kafka broker**의 내부 토픽(`__consumer_offsets`)을 사용하는 방식 권장.
- ZooKeeper 설정은 레거시 호환성을 위한 옵션일 뿐, 최신 버전에서는 사용하지 않음.
...
# ZooKeeper 설정 (Deprecated)
offset.storage=zookeeper
dual.commit.enabled=false
```

# Unix 철학: "Everything is a File"와 Kafka의 File Descriptor 활용

Unix 및 그 파생 운영체제에서 **"Everything is a file"**이라는 개념은 입력/출력을 처리하는 핵심 원리입니다. 이를 Kafka와 같은 시스템에서 활용하는 방식까지 확장해 살펴보겠습니다.

---

## "Everything is a File"의 개념

Unix 운영체제에서는 다음과 같은 다양한 자원들이 **파일처럼 다뤄집니다**:
...
### 동작 방식
1. **파일 디스크립터 생성**
   파일이 열리면 **파일 경로를 주소 체계로 사용하는 파일 디스크립터**가 생성됩니다.

2. **파일 디스크립터의 역할**
   파일 디스크립터는 파일 작업을 수행하는 데 필요한 **바이트 스트림 I/O 인터페이스**를 제공합니다. 익명 파이프, 네트워크 소켓 등 다양한 객체에도 동일하게 적용됩니다.

3. **확장된 개념: "Everything is a File Descriptor"**
   파일 디스크립터는 단순한 파일뿐 아니라 다양한 객체를 파일처럼 다룰 수 있도록 합니다.
   - 예: 프로세스 간 통신(익명 파이프), 네트워크 소켓.

---

## Kafka에서 File Descriptor의 역할

Kafka는 **file descriptors**를 두 가지 주요 목적으로 사용합니다:

1. **Log Segments**  
   Kafka는 데이터를 디스크에 저장하며, 각 파티션은 여러 **log segment 파일**로 나뉩니다.  
   - **파일 디스크립터 역할**:
     - 각 log segment 파일을 열고 관리하는 데 사용.
   - **필요한 디스크립터 수**:
     ```plaintext
     총 디스크립터 수 = (number_of_partitions) × (partition_size / segment_size)
     ```
     `segment_size` 설정이 작을수록 더 많은 log segment 파일이 생성되어 디스크립터 수가 증가합니다.

2. **Open Connections (Network Sockets)**  
   Kafka는 클라이언트(프로듀서, 컨슈머) 및 브로커 간 통신을 위해 **소켓(socket)**을 사용합니다.  
   - **소켓 디스크립터 역할**:
     - 클라이언트와의 네트워크 연결 관리.
     - 브로커 간 리플리케이션 통신 처리.
...
## Kafka File Descriptor 요구량 계산

Kafka 브로커에서 사용되는 총 file descriptor는 다음과 같이 계산할 수 있습니다:

```plaintext
총 File Descriptor = (Log Segments) + (Open Connections)
```
...
## Kafka 운영 및 "Everything is a File" 철학

#### Log Segments

```shell
Log Segments = (number_of_partitions) × (partition_size / segment_size)
...
## "Everything is a File"와 Kafka의 연결
Unix의 "Everything is a file" 철학은 Kafka에서 다음과 같이 구현됩니다:

- **Log Segments**: 각 세그먼트 파일은 Unix의 파일 디스크립터로 관리됩니다.
- **Open Connections**: Kafka의 네트워크 소켓도 파일 디스크립터로 추상화됩니다.

Kafka는 이 파일 디스크립터를 통해 **디스크 I/O**와 **네트워크 I/O**를 효율적으로 처리합니다.

---

## Kafka 운영 시 고려 사항

### 1. File Descriptor 제한
- 운영체제에서 사용할 수 있는 파일 디스크립터 수는 제한적입니다.
- Kafka 클러스터가 많은 파티션과 연결을 처리해야 하는 경우, **파일 디스크립터 제한 (`ulimit -n`)**을 늘려야 합니다.

### 2. Segment Size 설정
- `log.segment.bytes`를 조정하여 log segment 파일 수를 줄이고 디스크립터 사용량을 최적화할 수 있습니다.

cf) 
### 3. Connection 관리
- 클라이언트 수와 브로커 간 리플리케이션 연결을 고려하여 네트워크 소켓 디스크립터 사용을 최적화해야 합니다.

---

## 결론
Unix 철학의 **"Everything is a file"** 개념은 Kafka와 같은 분산 시스템에서 효율적인 파일 및 네트워크 I/O 처리 모델로 확장됩니다. 

- Kafka는 이 개념을 활용하여 **log segment 파일**과 **네트워크 소켓**을 동일한 방식으로 관리합니다.
- 운영자는 **파일 디스크립터 제한**을 적절히 조정하여 성능 문제를 방지해야 합니다.


```markdown


Kafka 주요 옵션 설정 요약 4가지 꼭 기억할것
...
메시지 보관 시간을 기본 7일(168시간)에서 3일(72시간)로 줄이면 디스크 공간 절약 가능.
실시간 처리가 주 목적이라면 보관 기간 축소를 권장.
- delete.topic.enable=true

토픽 삭제를 허용해야 디스크가 가득 찼을 때 즉각적인 대응 가능.
- allow.auto.create.topics=false

자동 토픽 생성을 비활성화하여 불필요한 토픽 생성 방지 및 안정적인 replication factor 관리.
- log.dirs=/data

메시지 저장 경로를 /tmp/kafka-logs 대신 별도의 대용량 디스크 경로로 설정해 디스크 부족 문제 예방.
```

### 
mutable vs immutable
- https://stackoverflow.com/questions/58224758/what-does-it-really-mean-by-kafka-partitions-are-immutable
- https://stackoverflow.com/questions/37914250/hadoop-immutable-vs-data-type-mutable


cf) committed 의 의미 및 replica.lag.max.messages 사용시 문제점
https://velog.io/@hyun6ik/Apache-Kafka-In-Sync-Replicas