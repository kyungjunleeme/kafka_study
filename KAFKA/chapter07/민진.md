...
1. `localhost:9092`에서 실행 중인 `kafka-docker-kafka-1` 컨테이너가 장애를 발생시키거나 종료됨
...
1. `localhost:9092`에서 실행 중인 `kafka-docker-kafka-1` 컨테이너가 복구되거나 재시작됨
...
spring:
  kafka:
    ...
    producer:
      ...
      acks: all
      retries: 3
      request.timeout.ms: 600000  # 요청 타임아웃 1.5초 설정
...
- `localhost:9092`가 장애를 일으키면서 클라이언트가 연결할 수 없게 되었음
...
- **메타데이터 갱신 지연**
  - 클라이언트가 장애 브로커(`localhost:9092`)를 감지하고 다른 브로커(`localhost:9093`)로 전환하려면 메타데이터가 최신 상태여야 하지만, 갱신이 지연되어 전환 실패
  - 메타데이터 갱신 주기가 너무 길거나, 갱신 요청이 장애로 인해 처리되지 않았을 가능성

- **파티션 리더 전환 실패**
  - `localhost:9093`이 새로운 파티션 리더로 전환되지 않았거나, 클라이언트가 새로운 리더 정보를 받지 못해 메시지 전송 실패
  - 이는 클러스터 내부의 리더 선출 지연 또는 클라이언트 메타데이터 갱신 실패 때문일 가능성
...
**예상결과** : 브로커의 응답 지연이 60초 이내일 경우 타임아웃 오류가 발생하지 않음

**[실패] 타임아웃 오류 발생** → 이론적으로는 될 것 같았는데.. 안된다
